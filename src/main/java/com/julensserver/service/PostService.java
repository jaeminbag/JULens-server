...
public class PostService {
    ...
    @Transactional(readOnly = true)
    public Page<PostListResponse> getPosts(String order, Pageable pageable) {
        if ("likes".equals(order)) {
            return postRepository.findAllWithLikeCountOrderByLikes(pageable)
        ...
        return postRepository.findAllWithLikeCountOrderByLatest(pageable)
                .map(p -> PostListResponse.from(
                        p.getPost(),
                        p.getLikeCount(),
                        p.getCommentCount()
